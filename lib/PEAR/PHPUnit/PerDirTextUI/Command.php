...
class PHPUnit_PerDirTextUI_Command
{
        ...
        foreach ($list as $path => $name) {
            if ($filterMasks) {
                foreach ($filterMasks as $mask) {
                    if (self::matchMask($mask, $name) || self::matchMask($mask, $path)) {
                        $testFiles[] = $path;
                        break;
                    }
                }
    ...
    {
        self::$_testDirs[realpath($dir)] = $fnSuffix;
     ...
     * 
     * @return array  Key - full pathname, value - test filename 
     *                with path relative to a directory added by
     *                addTestDir().
     */
    public static function getTestFiles($filterMasks = array())
    {
        ...
        if (count($filterMasks) == 1 && preg_match('{^(\w:)?[/\\\\][^*?]+$}s', $filterMasks[0])) {
	        ...
	        $files = array();
	        foreach (self::$_testDirs as $dir => $fnSuffix) {
	            $dir = str_replace('\\', '/', $dir);
	            $cwd = getcwd();
	            chdir($dir);
	            $elements = array();
